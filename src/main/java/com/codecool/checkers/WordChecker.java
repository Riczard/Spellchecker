...
public class WordChecker {
    ...
    private ArrayList<String> findSuggestion(String word) {
        ...
        suggestions.addAll(spaceInserted(word));
        return suggestions;
    }
    ...
    private ArrayList<String> spaceInserted(String input) {
        ArrayList<String> toReturn = new ArrayList<>();

        for (int i = 0; i < input.length(); i++) {
            String substring = input.substring(0, i);
            String nextSubstring = input.substring(i + 1);
            if (wordExists(substring)) {
                toReturn.add(substring);
            }
            if (wordExists(nextSubstring)) {
                toReturn.add(nextSubstring);
            }
        }
        return toReturn;
    }
